...
class ItemListViewModel: ObservableObject {
    ...
    @Published var viewMode: ItemListViewMode = .grid
    
    @Published var itemToShare: ShareableImage?
    @Published var isGeneratingShareImage = false
    
    var items: Results<CollectibleItem>
    
    private var token: NotificationToken?
    
    init(itemSet: ItemSet) {
        self.itemSet = itemSet
        self.items = itemSet.ownedItems.sorted(byKeyPath: "title")
    ...
    func prepareShareableImage() {
        ...
        Task {
            await MainActor.run {
                ...
                let viewToRender = ShareableSetView(itemSet: self.itemSet)
                ...
                let numberOfRows = ceil(Double(self.itemSet.ownedItems.count) / 3.0)
