...
class WishlistViewModel: ObservableObject {
    ...
    init(itemSet: ItemSet) {
        ...
        self.items = itemSet.wishListItems.sorted(byKeyPath: "title")
    ...
    func prepareShareableImage() {
        ...
        Task {
            await MainActor.run {
                let viewToRender = ShareableSetView(itemSet: self.itemSet, listType: .wishlist)
                ...
                let numberOfRows = ceil(Double(self.itemSet.wishListItems.count) / 3.0)
